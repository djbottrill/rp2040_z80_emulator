...
//BreakPoint switches
#define sw1 16                   //Breakpoints on / Off
#define sw2 17                   //Single Step push button
...
void portOut(uint8_t p, uint8_t v) {
  ...
  switch (p) {
    ...
    case GPP+2:
      ...
      digitalWrite(PB2, bitRead(v, 2));
      digitalWrite(PB3, bitRead(v, 3));
      digitalWrite(PB4, bitRead(v, 4));
      digitalWrite(PB5, bitRead(v, 5));
      digitalWrite(PB6, bitRead(v, 6));
      digitalWrite(PB7, bitRead(v, 7));
      break;
    case GPP+3:                               //GPP Direction Port
      ...
      if (bitRead(v, 2) == 1) pinMode(PB2, OUTPUT); else pinMode(PB2, INPUT_PULLUP);
      if (bitRead(v, 3) == 1) pinMode(PB3, OUTPUT); else pinMode(PB3, INPUT_PULLUP);
      if (bitRead(v, 4) == 1) pinMode(PB4, OUTPUT); else pinMode(PB4, INPUT_PULLUP);
      if (bitRead(v, 5) == 1) pinMode(PB5, OUTPUT); else pinMode(PB5, INPUT_PULLUP);
      if (bitRead(v, 6) == 1) pinMode(PB6, OUTPUT); else pinMode(PB6, INPUT_PULLUP);
...
uint8_t portIn(uint8_t p) {
  switch (p) {
    case GPP:        //Read GPIO port if Direction bit is 0 (Input)
      ...
      break;
    case GPP+2:      //Read GPIO port if Direction bit is 0 (Input)
      ...
      if (bitRead(pOut[GPP + 3], 2) == 0) bitWrite(pIn[GPP + 2], 2, digitalRead(PB2));
      if (bitRead(pOut[GPP + 3], 3) == 0) bitWrite(pIn[GPP + 2], 3, digitalRead(PB3));
      if (bitRead(pOut[GPP + 3], 4) == 0) bitWrite(pIn[GPP + 2], 4, digitalRead(PB4));
      if (bitRead(pOut[GPP + 3], 5) == 0) bitWrite(pIn[GPP + 2], 5, digitalRead(PB5));
      if (bitRead(pOut[GPP + 3], 6) == 0) bitWrite(pIn[GPP + 2], 6, digitalRead(PB6));
      if (bitRead(pOut[GPP + 3], 7) == 0) bitWrite(pIn[GPP + 2], 7, digitalRead(PB7));
      break;
